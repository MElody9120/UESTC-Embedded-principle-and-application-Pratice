...
#define LEN                  8
#define SHIFT                3
#define AXIS_X1            159    //0
#define AXIS_Y1             30
#define AXIS_X2             30    //1
#define AXIS_Y2             30
#define AXIS_X3             73    //2
#define AXIS_Y3             30
#define AXIS_X4             116   //3
#define AXIS_Y4             30
#define AXIS_X5             30    //4
#define AXIS_Y5             73
#define AXIS_X6             73    //5
#define AXIS_Y6             73
#define AXIS_X7            116    //6
#define AXIS_Y7             73
#define AXIS_X8             30      //7
#define AXIS_Y8            116
#define AXIS_X9             73    //8
#define AXIS_Y9            116
#define AXIS_X10           116      //9
#define AXIS_Y10           116
#define AXIS_X11           159
#define AXIS_Y11            73
#define AXIS_X12           159
...
void i2c_irq () {
    ...
    if ((csr & 0x800) && bNackValid) {                    /* NACK only valid in WRITE */
        ...
        } else if (i2c_state == I2C_STATE_WRITE) {    /* write data */
            if (i2c_data_pos < i2c_data_len) {
...
INT ts_location_test () {
    INT location[16][2] = {AXIS_X1, AXIS_Y1, AXIS_X2, AXIS_Y2, AXIS_X3, AXIS_Y3, AXIS_X4, AXIS_Y4, AXIS_X5, AXIS_Y5,
                           AXIS_X6, AXIS_Y6,
                           AXIS_X7, AXIS_Y7, AXIS_X8, AXIS_Y8, AXIS_X9, AXIS_Y9, AXIS_X10, AXIS_Y10, AXIS_X11, AXIS_Y11,
                           AXIS_X12, AXIS_Y12, AXIS_X13, AXIS_Y13, AXIS_X14, AXIS_Y14, AXIS_X15, AXIS_Y15, AXIS_X16,
                           AXIS_Y16};
    INT loc_idx;
    INT x = 0, i, j = 0, k = 0;
    INT x_pos, y_pos;
    INT t0;
    int n = 0;
    char num1[4] = {0, 0, 0, 0};  //???????????????(??????4¦Ë)
    char num2[4] = {0, 0, 0, 0};  //???????????????(??????4¦Ë)
    int num3, num4, num5;     //?????????int(??????4¦Ë)
    int flag = 0;             //?§Ý?????????????
...
VOID Draw_picture (VOID) {
	int Center_X,Center_Y,X,Y,i;
    while (Center_X == 159 && Center_Y == 210) {
        for (X = (Center_X - 5); X <= (Center_X + 5); X++) {
            Y = Center_Y;
            *(short *) (LCD_BUFF_BASE + (Y * 320 + X) * 2) = (short) 0xF800;
        }
        for (Y = (Center_Y - 3); Y <= (Center_Y + 8); Y++) {
            X = Center_X;
            *(short *) (LCD_BUFF_BASE + (Y * 320 + X) * 2) = (short) 0xF800;
        }
        for (Y = (Center_Y + 8); Y <= (Center_Y + 11); Y++) {
            X = Center_X - 5 + i;
            *(short *) (LCD_BUFF_BASE + (Y * 320 + X) * 2) = (short) 0xF800;
            i++;
        }
        ...
        for (Y = (Center_Y - 7); Y <= (Center_Y - 3); Y++) {
            X = Center_X - 4 + i;
            *(short *) (LCD_BUFF_BASE + (Y * 320 + X) * 2) = (short) 0xF800;
            i++;
        }
        ...
        i = 0;
        for (X = (Center_X - 2); X <= (Center_X + 1); X++) {
            Y = Center_Y - 5;
            *(short *) (LCD_BUFF_BASE + (Y * 320 + X) * 2) = (short) 0xF800;
        }
